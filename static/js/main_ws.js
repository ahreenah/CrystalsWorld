  ...
  this.rect=Crafty.e("2D, Canvas, Color, Mouse, Draggable")
    ...
    .bind('MouseUp', function(MouseEvent){
      ...
      if ((!isYourTurn)||(!readyState)||((mouseY>height-bottomBarHeight)
        ...
        //this.num=game.handNum;
        //game.handNum++;
        this.x=xpos;
        console.log('go back')
        x=xpos;
        this.y=ypos;
        game.hand.forEach(function(a){console.log(a);a.putBack()})
      }
      else{
        console.log('putted');
...
canvas.onmousemove=function(evt) {
    ...
    //console.log(mouseXUnit+' '+mouseYUnit);
}
...
function canPut(i,j,userId){
  if ((i<0)||(j<0)||(i>=fieldHeight)||(j>=fieldWidth))
    return false;
  // for local mode
  /*if ((userId==0) && (i==fieldHeight-1) && (j==0))
    return true;
  if ((userId==1) && (i==0) && (j==fieldWidth-1))
    return true;*/
...
function gameClass(){
  ...
  this.setHand=function(){
    this.hand=[
      /*card(100+cardDeltaX*0,cardRowY,cards[0],0),
      card(100+cardDeltaX*1,cardRowY,cards[1],1),
      card(100+cardDeltaX*2,cardRowY,new cardObject('Hyva',2,16,8,'r'),2),
      card(100+cardDeltaX*3,cardRowY,new cardObject('Paiva',3,14,9,'s'),3),
      card(100+cardDeltaX*4,cardRowY,new cardObject('Hyva',1,15,8,'d'),4)*/
    ];
  }
  this.moveCard=function(num){
    this.hand[num].x-=cardDeltaX;
    //this.hand[num].decXpos(cardDeltaX);
  }
  ...
  this.moveCards=function(num){
    ...
    for(let i=num; i<this.hand.length;i++){
      ...
      if (e.num>=num){
        console.log('moving '+e.num+' from x = '+e.x)
        e.num-=1;
        e.x-=(cardWidth+cardDeltaX)/2;
      }*/
...
function handShow(){
  //console.log('hand number: '+game.hand.length);
  //for(let i=0; i<game.hand.length; i++)
    //console.log(game.hand[i].num)
}
...
function endTurn(){
  //playerIdGlobal = 1 - playerIdGlobal;
  for(var i=0;i<fieldHeight; i++)
    for(var j=0;j<fieldWidth;j++)
      if (field.array[i][j]!=0)
...
function fieldObject(){
  ...
  this.drawCell=function(i,j){
    ...
    .bind("MouseDown",function(evt){
      ...
      //console.log('empty field is clicked! '+i+' '+j);
      if (
  ...
  this.setUnit=function(x,y,unitAttack,unitHealth,cobj,isOpponent){
    this.array[y][x]={attack:unitAttack,unitHealth:unitHealth,unit:new unit(x*fieldCellWidth,y*fieldCellHeight,cobj.name,cobj,y,x,isOpponent)}
  }
  this.clear=function(){
    for (var i=0; i<fieldHeight; i++){
      for(var j=0; j<fieldWidth; j++){
...
f_attack=function(field_selectedX,field_selectedY,mouse_XUnit,mouse_YUnit){
  ...
  try{
    console.log('unit: ');
    //console.log(field.array[mouse_YUnit][mouse_XUnit]);
    field.array[mouse_YUnit][mouse_XUnit].unit.damage(attack1);
  }//-=field.array[field.selectedX][field.selectedY].unit.attack;
  catch{
    field.array[mouse_YUnit][mouse_XUnit].unit.damage(attack1);
  }
...
function unit(xpos,ypos,text,cobj,fieldx,fieldy, isOpponent=false){ // существо
  ...
  this.select=function(){
    ...
    if (playerId!=playerIdGlobal){
      if (field.selected){
        if(!field.array[field.selectedX][field.selectedY].unit.canAttack){
          //console.log('Unit cannot attack');
          return;
        }
        //console.log('attack '+mouseYUnit+' '+mouseXUnit+' with '+field.selectedX+' '+field.selectedY);
        //console.log('sending attack '+field.selectedX+' '+field.selectedY+' '+mouseYUnit+' '+mouseXUnit);
        //sockSendMove(['move',field.selectedX,field.selectedY,mouseYUnit,mouseXUnit]);
        sendField();
        if (
          ...
          {//console.log('cannot attack so far')
          } 
        else{
          f_attack(field.selectedX,field.selectedY,mouseXUnit,mouseYUnit);
        }
      }
      //console.log('opponenet unit is clicked');
    }
    if ((!selected) && (playerId==playerIdGlobal)){
      ...
      //console.log(fieldx+' '+fieldy)//.unit.rect.color('grey')
      field.array[mouseYUnit][mouseXUnit].unit.rect.color('grey')
      field.selected=true;
      fieldx=mouseYUnit;
      fieldy=mouseXUnit;
      //console.log('selected '+fieldx+' '+fieldy)
      field.selectedX=fieldx;
      field.selectedY=fieldy;
      selected=true;
    }
    else{
      //console.log('unselecting '+selected+' '+playerId+' '+playerIdGlobal);
      try{
  ...
  this.hptext.bind('UpdateFrame', function(){
    //if (field.array[fieldx][fieldy].unit)
     // this.text(field.array[fieldx][fieldy].unit.health)
...
function addUnit(cobj,x,y,isOpponent=false){ // добавляет существо на стол
  //console.log(cobj.repr()+' added');
  //console.log('addUnit Called with : '+x+' '+y)
  field.setUnit(x,y,cobj.attack,cobj.health,cobj,isOpponent);
  sendField();
  //sockSendMove(['put',cobj,x,y])
  tableNum++;
}
...
function move(x1,y1,y2,x2,force=false){
  ...
  if (!force && !isYourTurn)
    return
  //console.log('moving '+x1+' '+y1+' '+x2+' '+y2)
  ...
  if ((Math.abs(x1-x2)>1 || Math.abs(y1-y2)>1)
     ...
     {
    //console.log('cannot move so far');
    return;
  }
  ...
  sendField();
  //if (!force)
    //sockSendMove(['move',x1,y1,x2,y2])
  //console.log(field.array[x1][y1])
  //console.log(field.array[x2][y2])
  //if (field.array[y1][x1]==0){}
  ...
  field.selectedY=y2;
  /*else{
    if (field.array[y2][x2]!=0){}
    else{
      field.array[y2][x2]=field.array[x1][y1];
      field.array[y1][x1]=0;
      field.array[y2][x2].unit.rect.x=x2*fieldCellWidth;
      field.array[y2][x2].unit.rect.y=y2*fieldCellHeight;
    }
  } */ 
}
...
async function doSelectDeck(){
  ...
  function doStart2(deckNum){
    doStart(deckNum);
    buttons.forEach(i=>{i.y-=1000})
    labels.forEach(i=>{i.y-=1000})
  }
  numberOfDecks=0;
  ...
  a.then(result=>{showDecks(result)},
  ...
  {
    let num = i;
    let deck2 = Crafty.e("2D, Canvas, Color, Mouse")
        .attr({x: 202, y: 112, w: 100, h: 30})
        .color('red')
        .bind('MouseDown', function () {
          doStart(j = i)
        })
    let label2 = Crafty.e("2D, Canvas, Color, Text")
        .attr({x: 205, y: 114, w: 100, h: 30})
        .textFont({size: '20px'})
        .textColor('white')
        .color('red')
        .text("Deck " + (i + 1))
...
var startButton=Crafty.e("2D, Canvas, Color, Mouse")
  ...
  .color('red')
  .bind('MouseDown',function(){doSelectDeck()})
...
var editDecksMenuBackButton=Crafty.e("2D, Canvas, Color, Mouse")
  ...
  .color('red')
  .bind('MouseDown',function(){showMenu();hideEditDecksMenu()})
...
var endGameButton=Crafty.e("2D, Canvas, Color, Mouse")
  ...
  .color('red')
  .bind('MouseDown',function(){hideEndMenu(); showMenu();})
...
function setField(list){// list of {fx:,fy:,co:'cardObject's}
  for(let i=0; i<fieldHeight; i++)
    for (let j=0; j<fieldWidth; j++)
      if(field.array[i][j]!=0){
  ...
  list.forEach((i)=>{
    field.array[i.fy][i.fx]={attack:i.co.attack,
...
function sendField(){
  ...
  readyState=false;
  //int=setInterval(()=>socket.emit('field',{gameId:gameId,field:data2,securityValue:securityValue}),1000)
  sendData={gameId:gameId,field:data2,securityValue:securityValue};
  //socket.emit('securityValue',securityValue)
